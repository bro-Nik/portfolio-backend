...
async def create_transaction(
    ...
    try:
        # Создаем транзакцию
        transaction = await transaction_service.create(user.id, transaction_data)

        # Получаем измененные активы на основе созданной транзакции
        portfolio_assets = await transaction_service.get_affected_portfolio_assets(
            user.id, [transaction]
        )
        wallet_assets = await transaction_service.get_affected_wallet_assets(
            user.id, [transaction]
        )
...
async def update_transaction(
    ...
    try:
        new_transaction, old_transaction = await transaction_service.update(
...
async def delete_transaction(
    ...
    """Удаление транзакции"""
    try:
        transaction = await transaction_service.delete(user.id, transaction_id)
        # Получаем измененные активы на основе созданной транзакции
        portfolio_assets = await transaction_service.get_affected_portfolio_assets(
            user.id, [transaction]
        )
